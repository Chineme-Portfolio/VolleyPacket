 ...
 * Receives live task status from the parent's SSE stream — no polling.
 */
...
const RESTARTABLE = new Set(["cancelled", "failed", "interrupted"]);
...
export default function TaskPanel({ jobId, taskKey, initialTask, canStart, isTerminal }: TaskPanelProps) {
  ...
  // Sync from parent (SSE updates flow through here)
  useEffect(() => {
    setTask(initialTask);
    ...
    // Allow restart for tasks in a terminal/restartable state
    if (RESTARTABLE.has(initialTask.status)) {
  ...
  const isRestartable = RESTARTABLE.has(task.status);

  // ── ACTION HANDLERS (optimistic updates — SSE confirms shortly after) ──
  ...
  async function handleStart() {
    ...
    try {
      ...
      if (taskKey === "pdfs") result = await startPdfs(jobId);
      else if (taskKey === "emails") result = await startEmails(jobId);
      else if (taskKey === "sms") result = await startSms(jobId);
      else result = await startPhotos(jobId);

      setTask(prev => ({ ...prev, status: "running", progress: 0, total: result.total, error: null }));
    } catch (err) {
  ...
  async function handlePause() {
    ...
    try {
      ...
      setTask(prev => ({ ...prev, phase: "paused" }));
    } catch (err) {
      toast(friendlyError(err));
    } finally {
      setActionLoading(null);
  ...
  async function handleResume() {
    ...
    try {
      ...
      setTask(prev => ({ ...prev, phase: "running" }));
    } catch (err) {
      toast(friendlyError(err));
    } finally {
      setActionLoading(null);
  ...
  async function handleCancel() {
    ...
    try {
      await cancelTask(jobId, taskKey);
      setTask(prev => ({ ...prev, status: "cancelled", phase: "cancelled" }));
      hasStartedRef.current = false;
    } catch (err) {
      toast(friendlyError(err));
    } finally {
  ...
  const showRestart = canStart && isRestartable && !isTerminal;
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isComplete ? "bg-green-50" : isRunning ? "bg-yellow-50" : isRestartable ? "bg-red-50" : "bg-gray-50"}`}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={isComplete ? "#047857" : isRunning ? "#b45309" : isRestartable ? "#dc2626" : "#6b7280"} strokeWidth="2">
              <path d={meta.icon} />
            </svg>
          </div>
          <div>
      ...
      {/* Progress bar */}
      {(isRunning || isPaused || isComplete || (isRestartable && task.total > 0)) && task.total > 0 && (
        <div className="mb-4">
          ...
          <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                isComplete ? "bg-green-500" : isPaused ? "bg-yellow-400" : isRestartable ? "bg-red-400" : "bg-green-600"
              }`}
      ...
      <div className="flex items-center gap-2 mt-4">
        ...
        {showRestart && (
          <button
            ...
            className="flex-1 px-4 py-2 text-sm font-medium text-white bg-green-800 rounded-xl hover:bg-green-900 transition-colors disabled:opacity-50"
          >
            {actionLoading === "start" ? "Starting..." : `Restart ${meta.label}`}
          </button>
        )}
